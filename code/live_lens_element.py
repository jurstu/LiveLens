...
class LiveLensElement:
    ...
    def generate_world(self, fill_world, start_x=0, start_y=0, start_z=0, clear_world=True):
        if clear_world:
            self.world.clear()
...
if __name__ == "__main__":
    ...
    from live_lens_world_generators import generate_axis_world, generate_marker_world

    lle = LiveLensElement(resolution=(640, 480, 30))
    lle.generate_world(generate_marker_world, 4, 0, 0)
    lle.generate_world(generate_axis_world, clear_world=False)
    ...
    if frame is not None:
        ...
        imageio.mimsave("output.webp", frames, duration=1 / 60)
